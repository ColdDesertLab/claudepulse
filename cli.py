...
def require_db():
    if not DB_PATH.exists():
        print("Database not found. Run: claudepulse scan")
        sys.exit(1)
...
def require_db_msg():
    return "Database not found. Run: claudepulse scan"
...
USAGE = """
claudepulse - Claude Code usage dashboard

Usage:
  claudepulse scan       Scan JSONL files and update database
  claudepulse today      Show today's usage summary
  claudepulse stats      Show all-time statistics
  claudepulse dashboard  Scan + start dashboard at http://localhost:8080
...
def main():
    if len(sys.argv) >= 2 and sys.argv[1] in ("-v", "--version"):
        print(f"claudepulse {__version__}")
        return
